...
- 로그인은 **SSH 키**(권장) 또는 비번. 키는 한 번만 등록하면 이후 무비번:
  ```bash
  ./setup-keys.sh servers/*.conf     # 서버당 비번 1회(또는 sshpass 로 자동) → 이후 무비번
  ```
- 비번 로그인을 쓰려면 `--pw-login`(conf 의 SUDO_PASSWORD 재사용) 또는 conf 의
  `SSH_LOGIN_PASSWORD` — 둘 다 **sshpass** 필요 (mac: `brew install hudochenkov/sshpass/sshpass`)
